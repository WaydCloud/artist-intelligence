...
3. **스템 분리 구현**(D-034 ③): **정의·수용조건 문서화 완료(2026-07-30)** — 위 🟢 절 참조. 남은 것은 `demucs` 설치 승인 → 구현 → TESTS §7.2 게이트 → 통과 축만 genre-impulse 검출 규칙으로 승격(v1.1).
...
| **스네어 축 유효성 게이트** | 대비 1.71 | `--snare-min-contrast` (스템) |
...
## 🟢 스템 분리 — **지표 정의·수용조건 완료(2026-07-30), 구현 대기** (D-034 ③)

- **문서 먼저 단계는 끝났다**(D-034 ③이 건 조건). 정의 = [`modules/sonic-profile/RULES.md`](modules/sonic-profile/RULES.md) **§3.8**(축 12종 + 관습값 4종) · 수용조건 = [`TESTS.md`](modules/sonic-profile/TESTS.md) **§7.2**(합성 픽스처 14항 + 정답지 채택 게이트 4축 + 분리 품질 회귀) · 범위 = [`SPEC.md`](modules/sonic-profile/SPEC.md) §6.
- **Demucs 4.1.0 `htdemucs`**(4스템 @44.1kHz). `melband-roformer-infer`는 **2스템뿐이라 드럼이 안 나온다** — 목적에 안 맞는다. 라이선스 **코드·가중치 모두 MIT**(상업 전환 부채 아님).
- **여는 것 3종**: 저지클럽 **하프타임 스네어**(`halftime_snare_ratio`) · 드릴 **슬라이딩 808**(`bass_glide_ratio`) · 하이퍼팝 **보컬 처리**(`vocal_tuning_hardness` 등). genre-impulse RULES §2.2의 해제 조건이 이 셋을 가리키도록 갱신됨.
- ⚠ **하이햇 롤은 안 열린다** — 16칸 격자의 해상도 문제라 스템과 무관하다. 둘을 묶어 두지 말 것.
- **구현 시 주의 3가지**: ① **무보관 불변식이 스템에도 적용된다**(스템은 오디오다 — 임시 파일로 떨어뜨리지 말 것, 예외 경로 포함) ② **디코드 경로 이원화** — 분리는 44.1kHz 스테레오, 지표 계산은 분리 후 **22050 모노로 되돌려** 기존 축 정의를 유지 ③ 엔진 키에 `stem_set` 추가 → **콜드 실행 1회**(안 넣으면 스템 축이 빈 옛 레코드가 되살아난다).
- **다음 액션**: 의존성 `demucs`(+13) 설치 승인 → 구현 → TESTS §7.2 전항 → **채택 게이트에서 판별력이 없으면 원장에 "없다"고 적는다**(§3.7.1 규율). 수집 +20~25분/일은 게이트 통과 후에만 지불한다.

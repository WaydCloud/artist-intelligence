...
## 🧭 다음 행선지 — **인터랙션(크로스헤어·툴팁)** (재개점)

> **시각화 계약은 6모듈 전부 적용 완료**(D-044). `ADOPTED_MODULES`가 전부라 R1~R8 + 구획이 이제 모든 탭에서 하드 게이트다.
> 계약 = D-041 · 채택 경계 = D-042 · 구획 = D-043 · **6탭 확산 + 값의 방향 규율 = D-044** · 형식 정본 = [`DESIGN.md`](DESIGN.md) §7.1~§7.5.
...
2. **막대·셀 툴팁** — 위와 같은 프리미티브로. 현재 `title` 속성이라 마우스를 올려야 하고 키보드로는 닿지 않는다.
3. **모션** — `motion`은 설치됐고 추론 펼침·구획 전환에만 쓰인다. 상태 변화를 설명하는 곳에만 추가한다(공식 `frontend-design` 경고를 §7이 승계: 과한 애니메이션은 AI 생성처럼 읽힌다).
4. **지표 타일 편집** — sonic-profile은 아직 타일 24개에 구획이 없다. 어느 축을 표면에 남길지는 도메인 판단이다.
5. **긴 목록의 형태** — signal-bridge의 막대 3종은 상위 30행으로 자르고 그 사실을 적는다(§7.5). 30을 넘는 목록을 계속 막대로 둘지, 표·검색으로 바꿀지는 아직 답하지 않았다.

⚠ **눈으로만 잡히는 결함이 두 라운드에 걸쳐 열한 건 나왔다** — 전부 정적 게이트 4종과 탭 스모크를 **통과한 상태**였다. 1차(파일럿)는 색 순환·기준 링 겹침·결측 마커 위치·순위를 막대 길이로·타일 값 잘림. 2차(6탭 확산)는 **부호를 막대에·역방향 축을 막대에·격자 색 방향 반대·미관측 구간을 0으로·계정명 노출·저장 키 노출**. 규율은 §7.3~§7.5에 있고, 공통점은 **그림의 방향과 주장의 방향이 어긋난 것**이다.

**탭마다, 구획마다 렌더를 실제로 볼 것.** 정적 게이트는 열한 건 중 하나도 잡지 못했다.
...
4. 게이트: `validate_report_data.py`(+`--selftest`) · `ruff` · `pyright` · `tsc` · `next lint` · `npm run smoke:tabs`(구획까지 돈다) · **라이트/다크 육안**.
...
- ✅ **git 정리 완료**: 열린 PR **0**. 최근 세션 이력은 [`Handoffs/2026-07-30-visualization-six-tabs.md`](Handoffs/2026-07-30-visualization-six-tabs.md)(남은 4탭 확산 → 6/6 채택 · 값의 방향 규율), 그 앞이 [`Handoffs/2026-07-30-visualization-pilot-and-chart-history.md`](Handoffs/2026-07-30-visualization-pilot-and-chart-history.md)(시각화 착수 → 두 탭 적용).

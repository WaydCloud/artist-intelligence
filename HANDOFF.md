...
## 🧭 다음 행선지 — 장르 임펄스: **기획 완료(D-033), 승인 대기 → 승인 시 Phase A 조사 착수**

> **① 의도 파악 · ② 조사 전략 · ③ 기획 완료**(2026-07-29). 작업 정본 = [`docs/PLAN-genre-impulse.md`](docs/PLAN-genre-impulse.md), 프레임 4결정 = [`docs/DECISIONS.md`](docs/DECISIONS.md) **D-033**. **구현 금지는 유지** — Phase C(모니터 모듈)는 케이스북 ≥6건(음성 ≥2) + 도메인 소유자 승인 전 착수 금지.

- **프레임(D-033)**: 시장 = 이중 축 매트릭스(지리×확산) · 임펄스 = 특질 벡터+장르명 라벨 · 속도 기준선 = 과거 사례 리드타임 분포에서 도출 · 소급 = 미국 외 발원 경로 포함(기간 2018~ 기본, 튜닝 가능).
- **다음 액션 = 도메인 소유자 확인 5건**(PLAN §6): ① 케이스 후보 10건 목록 확정(저지클럽 앵커 + 음성 사례 하이퍼팝·드릴 포함) ② 스템 분리 승인(별건 대기 중 — 저지클럽 서명 추출이 이 뒤에 있음) ③ 격자 32칸 확장 검토 ④ 신규 데이터 레그(빌보드 프록시·틱톡, 건별) ⑤ 조사 기간 하한.
- 승인되면 **Phase A**(조사 ≥80%): A1 케이스 타임라인 → A2 사운드 서명(기존 sonic-profile 프리뷰 파이프라인 소급 사용 — 신규 구현 아님) → A3 리드타임 분포 → A4 조기 신호 소스 순위. 상세·완료조건은 PLAN §3·§7.

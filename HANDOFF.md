...
# 🔵 수집 이전 — **워크플로는 다 섰다. 남은 것은 시크릿 넷과 첫 실행이다** (2026-08-01 저녁)
...
✅ **확인된 것**(run `30683849417` · 3분 22초 · **읽기 전용으로 끝까지 관통**):

| 스텝 | 결과 |
|---|---|
| 모드 가드 | `MODE = read-only` · `RUNNER_MODE=(unset) -> AI_DRYRUN=1 LIVE_MODE=0` |
| 복원 | **1,627파일 / 54아카이브 / 14.33MB** (로컬과 동일) |
| 트리 게이트 | `chart dates=16 · sonic 7` |
| 수집기 | 실행됨 (오늘은 로컬 완주라 `no-op`) |
| 계약 게이트 | **6리포트 · chart data CLEAN** |
| 백업 · 되밀기 | **스킵** (읽기 전용이므로 의도된 것) |

⏳ **아직 안 재진 것**: 수집·파생 레그가 **실제로 도는 것** · 콜드 캐시 비용(모델 ~500MB) · sonic 레그.
...
**㉱ 병행 운전** — 로컬 Task Scheduler를 그대로 두고 하루를 대조한다. **내일(08-02) 08:17 KST에 러너가 처음으로 실제 수집을 한다.**
- 🔴 **로컬보다 먼저 돌지 않으면 대조가 성립하지 않는다.** 로컬이 09:00에 `done=true`를 1층에 올리므로 09:17 러너는 그것을 복원해 **매일 no-op으로 나간다**(08-01 세 번의 실행이 전부 그랬다). 그래서 병행 기간 한정으로 `17 23 * * *`(08:17 KST) 한 줄을 열어 뒀다.
- 🔴 러너 시계를 **`tzutil`로 KST에 박았다** — `$today`가 스냅샷 파일명·재개 상태·시도 원장·백업 키를 전부 정하는데 러너 기본은 UTC라, 09:00 KST보다 앞선 슬롯은 **하루 전 날짜로** 찍힌다. 상시 크론(00:17\~06:17 UTC)에서 날짜가 맞던 것은 **우연이었다**. 크론 표 자체는 여전히 UTC로 읽힌다.
- 볼 것: 러너 `report.json` vs 로컬 것. **바이트 일치를 기대하지 않는다**(43분 차라 발행 주기가 다를 수 있다). 묻는 것은 "러너에서 수집·파생·sonic 레그가 실제로 도는가"다.
...
**㉲ 전환** — 아래 셋을 **같은 날** 한다. 순서가 어긋나면 하루 $6이 나가거나 하루가 통째로 빈다.

1. 🔴 **`daily-collect.yml`에서 `- cron: "17 23 * * *"` 한 줄을 지운다.** 병행 기간에는 러너가 아무것도 쓰지 않아 08:17에 받은 것이 한 발행 주기 뒤처져도 버려진다. **전환 후에는 1층에 쓰므로 어제 보드를 오늘 날짜로 박제한다** — 1층은 재취득 불가다. 로컬이 09:00에 성공해 온 것이 우리가 아는 유일한 안전 시각이다.

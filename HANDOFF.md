...
# 🚀 SaaS 전환 — 결정 4건 + 프리미엄 프런트 v1 (2026-07-31)

> 📄 결정 근거 = **[`docs/DECISIONS.md`](docs/DECISIONS.md) D-056**(방향 4결정) · **D-057**(프런트 6결정) · 규율 = **[`DESIGN.md`](DESIGN.md) §2.1 굵기 · §7.8 스크롤리텔링**.

### ✅ 이번에 닫힌 것

**방향 4결정**(D-056). 셋이 같은 방향을 가리켰다 — **자원을 프런트에 몰아 주는 답**이다.

| 결정 | 답 | 그래서 무엇이 바뀌나 |
|---|---|---|
| 누가 쓰는가 | **공개 데모 · 포트폴리오 우선** | 🔴 **`AGENTS.md` §3을 고치지 않는다.** 인증·DB·다중 테넌시·과금이 전부 불필요. 정적 export 유지 |
| 무엇을 파는가 | **리포트 열람 + 온디맨드 분석** | 열람이 먼저, 온디맨드가 그 뒤(Modal은 그때 정당해진다). 데이터 접근은 빠졌다 |
| 스크롤리텔링 경계 | **결론 유도까지 허용** | 불변식은 살고 **결론이 꼬리표를 달고 나온다**(D-039 선례) |
| 브랜드 | **지금 `DESIGN.md` 위에 얹는다** | 팔레트 재검증 불필요. 새 표면에도 §7.3~§7.7이 처음부터 걸린다 |

**프런트는 섹션 단위로 끊어 도메인 소유자와 상의하며 만들었다**(A 히어로 → B 무대 → C 문장 → F 대시보드 → F-1 테마 → E 레일 → G 보조 표면). 결정 여섯은 D-057.

| 표면 | 지금 상태 |
|---|---|
| `/` | 스크롤리텔링 **7단계**(관측 5 + 해석 2) + `답하지 않는 것` 전용 화면 + 질문을 머리글로 쓰는 모듈 카드 6종 |
| `/artist-intelligence` | 굵기 규칙 적용 완료. 내용 변경 없음 |
| `/utilities` | **기준 원장**(빈 페이지였다). 임계값 10개 · 모듈별 · 돌려 볼 탭으로 링크 |
| `/labs` | 머리부를 다른 표면과 같은 모양으로. 경고가 제목 자리로 올라와 더 크게 읽힌다 |

- 서사 조달 `lib/story.ts` · 기준 조달 `lib/criteria.ts` · 무대 `components/story/NarrowingField.tsx` · 서사 `components/story/LeadNarrative.tsx` · 내비 `components/story/LandingNav.tsx` · 테마 `components/ThemeProvider.tsx`.
- 🔴 **수치는 전부 `report.json`에서 구조적으로 온다. 없으면 빌드가 선다.** 부분집합 순서(`통과 ≤ 표본 ≤ 선행`)와 구성의 합도 검사한다. 인사이트 문자열은 파싱하지 않는다.
- 모듈 변경 하나: signal-bridge에 지표 **`표본 충족 선행`** 추가(원장 §3.1이 "두 축을 섞지 말라"고 적어 둔 것을 화면이 어기고 있었다). 서사가 `126 → 65 → 12 → 1`이 됐다.
- `motion/react`를 새로 늘리지 않았다(CSS 전환만 · §7.7 기준선 3파일 유지).

### 🔴 이번에도 육안·콘솔로만 잡힌 것 여덟 (스물여덟째~서른째 건)

정적 게이트 4종과 타입 검사를 **전부 통과한 상태**였다. §7.5의 경고가 그대로 재현됐다: **새 표면을 만들면 처음부터 다시 걸린다.**

1. 🔴 **무대가 통째로 안 보였다.** `main`의 `overflow-hidden`이 조상에 걸리면 **`position: sticky`가 죽는다**(스크롤포트가 그 조상으로 바뀐다). 콘솔 에러 0 · 타입 통과 · 스텝 전환도 정상이었고 **오른쪽 칸만 비어 있었다.**
2. 🔴 **테마가 표면마다 끊겼다.** 네 표면 중 둘(`/utilities` 아예 안 걸림 · `/labs` 토글 없음)이었고 랜딩에서도 같은 사고가 났다. **세 번 났으면 실수가 아니라 구조다** → 루트 공급자로 올렸다(D-057 ⑤).
3. 🔴 **하이드레이션이 통째로 깨졌다.** 공급자가 첫 렌더에서 저장값을 읽자 토글 라벨이 정적 산출물과 어긋났다. **스크린샷은 멀쩡했고 콘솔 로그로만 잡혔다.** 규칙: **색은 페인트 전에, 상태는 마운트 뒤에.**
4. 🔴 **라이트 메탈의 밝은 정거장이 대비 미달**(2.44:1 · 기준 3:1). 굵을 때는 눈으로 버티다가 워드마크를 100으로 내리자 실제로 사라졌다.
5. **걸러진 칸과 범위 밖 칸이 구별되지 않았다.** 같은 색에 흐림만 달리 줬더니 라이트에서 둘 다 흰 배경으로 씻겼다. **구분은 불투명도가 아니라 색이 진다.**
6. **한글에 라틴용 자간이 걸렸다.** `tracking-[0.28em]`이 `지 금  도 는  관 측`으로 벌어졌다. 넓은 자간은 **라틴 표기 전용**이다.
7. **작은 워드마크를 히어로 것을 줄여서 썼다.** 14px·10px에 굵기 100 + 메탈은 유령이 된다. **같은 실수를 두 곳에서 했다**(랜딩 내비 · 대시보드 헤더).
8. **`flex-col-reverse`에 `justify-end`를 빠뜨려** 좁은 폭에서 지표 숫자의 밑줄이 어긋났다. 데스크톱을 고치면서 모바일을 만들었다.

부수: 범례가 있는 단계에서만 자리를 차지해 **스크롤 내내 무대가 위아래로 뛰던 것**(자리를 비워 둔 채 유지) · 한국어가 단어 중간에서 접히던 것(`break-keep`) · `<th>`·`<b>`는 브라우저 기본이 bold라 **명시하지 않으면 굵기 규칙 밖에 남는다**.

### 🔴 다음 세션이 처음 할 일

1. **결정 3(수집 이전)** — 이것만은 결정 1이 풀어 주지 않았다. SaaS 때문이 아니라 **백업 없는 단일 디스크** 때문이다. `data/live/`는 gitignore라 관측 이력이 이 PC에만 있다(65MB). GitHub Actions cron에서 **유료 키·모델 캐시·멜론 MCP 레그**가 각각 어떻게 되는지 실측이 먼저다.
2. **서사가 이름을 못 부른다** — 두 관문을 통과한 1팀이 누구인지가 **인사이트 문자열에만** 있어 랜딩이 구조적으로 조달하지 못한다(규율상 파싱하지 않았다). signal-bridge가 그 팀을 **구조적 필드로 싣게** 하면 서사의 마지막 단계가 이름으로 닫힌다.
3. **genre-impulse 노브 라벨에 표기가 샜다** — `하위 백분위 P_low`·`상위 백분위 P_high`가 기준 원장 화면에 그대로 뜬다. 모듈이 쓴 문구라 **모듈 쪽에서 고칠 일**이다(화면에서 고치면 원장과 어긋난다).
4. **두 번째 서사를 만들 때 프리미티브를 뽑는다.** 지금은 하나뿐이라 일부러 일반화하지 않았다(§1: 더하기 전에 뺄 것을 먼저).
5. 결정 4(저작권)는 **급함이 줄었다** — 공개 데모는 지금과 같은 회색이다. 온디맨드나 유료화가 서면 다시 1순위. 🔺 리드타임이 길어 그때는 먼저 걸어야 한다.

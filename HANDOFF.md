...
4. 🔴 **스네어 유효성 게이트 임계 1.71의 근거를 고칠 것** — 이건 **저역** 기준선을 **중역** 축에 옮겨 쓴 것인데, 코호트 실측(n=123)에서 1.71은 두 분포 모두의 **약 58백분위**다. 바닥이 아니라 한복판을 자르는 컷이라 정답지 3곡이 평범한 값(1.35·1.59·1.37)으로 결측 처리됐다. **분포 하단(p10 ≈ 1.33)에서 도출**한 뒤 하프타임 게이트 재실행. 이건 값이 아니라 **형식의 결함이라 엔지니어 소유**다(AGENTS §2.1).
5. **보컬 처리 축 재정의** — 절대 격자 정렬(철회됨) 대신 **지속 노트 안의 f0 분산**. 격자 오프셋에 불변이라 해상도 의존이 원리적으로 약하다. 사전 등록 → 정답지 검증 → 등재 순서를 다시 밟는다.
6. **드릴 새 가설 사전 등록** — 슬라이딩 808은 실측 기각됐다(3/3 중앙). 결과를 보고 규칙을 만들지 않는다는 규율상, 새 가설을 **먼저** 등록해야 한다. 유력 후보는 아래 C(격자 해상도)다.

### C. 마디 격자 32칸 — **두 케이스의 공통 병목** (⚠ 승인 필요 · 규모 큼)

7. 저지클럽 **하이햇 롤**과 드릴 후보가 모두 여기 걸린다. 16칸으로는 32분·트리플렛이 원리적으로 안 보인다(앨리어싱). **영향 범위가 넓다**: `kick_bar_profile` 정의·저장 형식·템플릿 원장(§3.1.5)·소급 재계산 경로 전부. 스템처럼 "정의 먼저 → 수용조건 먼저 → 구현" 순서를 밟고 별도 결정으로 남길 것.
...
- 🔴 **게이트 1차: 통과 축 0.** 판정 정본 = [`data/research/genre-impulse/stem_gate_result.json`](data/research/genre-impulse/stem_gate_result.json), 상세 = RULES **§3.8.4.1**, 재현 = `python scripts/stem_gate.py`. **실패가 두 종류이고 후속이 다르다**:
  ...
  - **판정 불가**(원인부터 고칠 것): `halftime_snare_ratio` — 유효성 게이트(대비 1.71)가 정답지 5곡 중 3곡을 떨어뜨려 기준을 넘을 방법이 없었다(측정된 2곡은 P77.3, 컷 P80 바로 아래). **임계 재조정(도메인 소유자) 또는 정답지 확대 후 재실행.** `vocal_tuning_hardness` — 🔴 **pyin의 격자를 재고 있었다**(122곡·고유값 6·정확히 10센트 간격 = `librosa.pyin` 기본 `resolution=0.1`). 같은 함수를 쓰는 `bass_note_stability`도 동일(고유값 9/123).

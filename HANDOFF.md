...
## 🧭 다음 행선지 — **`sonic-profile`을 구획으로 가르기** (재개점)

> **모션은 끝났다**(D-046). 쓰는 곳 넷 · 기각 목록 여섯 · `prefers-reduced-motion`을 `globals.css` 한 곳에서 집행. 규율 정본 [`DESIGN.md`](DESIGN.md) §7.7.
> **인터랙션도 붙어 있다**(D-045) — 크로스헤어 + 통합 툴팁 + 키보드 경로, `smoke:tabs`가 센다.
> 계약 = D-041 · 채택 경계 = D-042 · 구획 = D-043 · 값의 방향 = D-044 · 인터랙션 = D-045 · **모션 = D-046** · 형식 정본 = `DESIGN.md` §7.1~§7.7.

**왜 여기인가**: 여섯 탭 중 **`sonic-profile`만 구획이 없다**. 카드 17개 · 지표 타일 24개가 한 줄로 쏟아져 `main` 본문이 **28,357자**다 — 구획을 쓰는 다른 탭은 구획당 2,400~9,900자다. D-043("한 화면에 최대 3")이 파일럿 탭에만 닿지 않은 상태이고, 남은 시각화 작업 중 **도메인 소유자 답을 기다리지 않고 시작할 수 있는 유일한 것**이다.

- **엔지니어가 소유하는 것**: 구획 구조(질문 단위로 가르기 · `_SECTIONS` 표 · 차트를 구획에 배정 · 상한 3 검사 통과).
- **도메인 소유자가 소유하는 것**: 어느 축을 표면에 남길지(타일 24개 중). 구조를 먼저 세우고 배치안을 들고 물으면 답하기 쉬운 질문이 된다.
...
1. **지표 타일 편집 = `sonic-profile` 구획 가르기** (위 재개점). 다른 다섯 탭은 이미 구획으로 서 있다.
...
4. ~~**모션**~~ — ✅ 완료(D-046). 쓰는 곳 넷(구획 전환 · 추론 펼침 · 노브→막대 폭 · 작업 결과 메시지) · **기각 목록 여섯** · `prefers-reduced-motion`은 `globals.css` 한 곳에서 집행. 규율 §7.7.

⚠ **눈으로만 잡히는 결함이 네 라운드에 걸쳐 열일곱 건 나왔다** — 전부 정적 게이트 4종과 탭 스모크를 **통과한 상태**였다.
...
**탭마다, 구획마다 렌더를 실제로 볼 것.** 열일곱 건 중 정적 게이트가 잡은 것은 없다.
...
- ✅ **git 정리 완료**: 열린 PR **0**. 최근 세션 이력은 [`Handoffs/2026-07-30-motion-discipline.md`](Handoffs/2026-07-30-motion-discipline.md)(모션 규율 + reduced-motion 안전망), 그 앞이 [`Handoffs/2026-07-30-interaction-crosshair-tooltip.md`](Handoffs/2026-07-30-interaction-crosshair-tooltip.md)(인터랙션 + 스모크 게이트), 그 앞이 [`Handoffs/2026-07-30-visualization-six-tabs.md`](Handoffs/2026-07-30-visualization-six-tabs.md)(6/6 채택 · 값의 방향), 그 앞이 [`Handoffs/2026-07-30-visualization-pilot-and-chart-history.md`](Handoffs/2026-07-30-visualization-pilot-and-chart-history.md)(시각화 착수 → 두 탭).
...
| **긴 목록의 형태** | `leadlag` 튜너가 **115행을 세로로 쏟는다**(스크롤 세 화면). 막대로 둘지 표·검색·정렬로 바꿀지는 "이 목록을 어떻게 쓰는가"에 달렸다 — 훑는 것이면 표, 상위만 보는 것이면 자르는 것이 맞다(D-045 딸림). **여기에 정렬 규칙 질문이 붙었다**(D-046): 임계를 움직이면 순서가 뒤집혀 눈이 행을 놓치는데, 그것은 모션으로 풀 수 없다(§7.7). 순서를 값에 맡길지 고정할지가 같은 질문의 뒷면이다 |

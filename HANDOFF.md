...
# 🚀 방향 전환 — SaaS · 프리미엄 프런트 (2026-07-31 지시)

> **도메인 소유자 지시**: 이것을 **SaaS·플랫폼**으로 만들고 **스크롤리텔링 등 SOTA 수준의 프리미엄 프런트엔드**를 갖춘다.
>
> 📄 **착수 문서 = [`docs/DRAFT-saas-platform.md`](docs/DRAFT-saas-platform.md)** — 딛고 설 자산 · 먼저 풀 것 · 결정 6건 · 제안 순서가 거기 있다. **다음 세션은 그 문서를 먼저 읽는다.**

### 🔴 다음 세션이 처음 할 일 (순서 그대로)

1. **결정 1·2를 도메인 소유자에게 먼저 묻는다** — **누가 쓰는가**(사내 전용 / 외부 판매 / 조직 수)와 **무엇을 파는가**(리포트 열람 / 온디맨드 분석 / 데이터 접근). 다중 테넌시·인증·가격·아키텍처가 전부 여기서 갈린다. **답 없이 아키텍처를 고르면 되돌린다.**
2. 답을 기다리는 동안 막히지 않는 것 둘: **수집 이전 조사**(GitHub Actions에서 유료 키·모델 캐시·MCP 레그가 각각 어떻게 되는지 실측) · **스크롤리텔링 파일럿 대상 탭 고르기**.
3. 🔴 **`AGENTS.md` §3(정적 우선)을 고치기 전에 `docs/DECISIONS.md`에 새 결정을 먼저 적는다.** 규칙을 먼저 지우고 근거를 나중에 적으면 왜 그렇게 정했는지가 사라진다.

### 이 전환에서 놓치면 안 되는 것 셋

- ✅ **미결이던 "상시 서버" 논의가 이 지시로 풀린다.** 서버가 정당해지는 조건 3개(여러 사람·온디맨드·규모)를 **SaaS가 동시에 참으로 만든다.** 그러므로 첫 결정은 "서버를 둘 것인가"가 아니라 **"어떤 서버를, 무엇까지"**다.
- 🔴 **가장 실질적인 병목은 수집이 이 PC 한 대에 묶여 있는 것**이다. `data/live/`는 gitignore라 **관측 이력이 이 PC에만 있다**(65MB, 백업 없음). 멜론 MCP 레그는 대화형 인증이라 헤드리스로 안 따라온다.
- 🔴 **스크롤리텔링과 책임소재 불변식의 긴장이 이 전환의 핵심 설계 질문이다.** 결론으로 데려가는 서사는 **평결 배달**이라 금지되고, **증거를 순서 지어 보이는 서사**는 양립한다. 구획(D-043)이 이미 그 형태에 가깝다는 것이 출발점이다. 강제 스크롤 금지 · `reliability`·`notAnswered`가 각주로 밀리면 §5 위반.
- 🔺 **저작권 검토는 리드타임이 길어 먼저 걸어야 한다** — 한국 저작권법에 TDM 면책이 없고, **유료화가 지금 회색의 성격을 바꾼다.**

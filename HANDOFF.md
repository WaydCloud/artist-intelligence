...
## 🧭 다음 행선지 — **도메인 소유자 답 3건을 받아 마무리** (재개점)

> **시각화 엔지니어링은 여기서 일단락됐다.** 여섯 탭 전부 구획(D-047) · 계약 6/6(D-044) · 인터랙션(D-045) · 모션(D-046).
> 계약 = D-041 · 채택 경계 = D-042 · 구획 = D-043 · 값의 방향 = D-044 · 인터랙션 = D-045 · 모션 = D-046 · **`sonic-profile` 구획 + 앵커 = D-047** · 형식 정본 = [`DESIGN.md`](DESIGN.md) §7.1~§7.7.

**남은 것은 전부 "값"이고 값은 도메인 소유자 소유다**(AGENTS §2.1). 아래 결정 대기 표의 세 건(긴 목록의 형태 · 용어 설명의 자리 · 타일 24개 중 무엇을 남길지)을 받으면 시각화가 닫힌다. 답을 기다리는 동안은 **대기 목록**(아래)의 행 늘리는 작업이 병행 가능하다.

> ⚠ 세 건 다 **화면을 보고 뜬 질문**이다. 물을 때 지금 화면을 함께 보여주면 답이 빨라진다.
...
### 남은 시각화 작업 (전부 도메인 소유자 답 대기)

1. ~~**`sonic-profile` 구획 가르기**~~ — ✅ 완료(D-047). 여섯 구획 · 타일 24개 배정 완료. **남은 것은 "24개 중 무엇을 표면에 남길지"**이고 그것이 도메인 판단이다(자리는 정해졌다).
...
⚠ **눈으로만 잡히는 결함이 다섯 라운드에 걸쳐 열아홉 건 나왔다** — 전부 정적 게이트 4종과 탭 스모크를 **통과한 상태**였다.
...
**탭마다, 구획마다 렌더를 실제로 볼 것.** 열아홉 건 중 정적 게이트가 잡은 것은 없다.
...
| **타일 24개 중 무엇을 남길지** | `sonic-profile`의 중앙값 타일 22개 + 표본 2개. **자리는 정해졌다**(D-047: 여섯 구획에 6·7·5·0·0·6으로 배정) — 남은 것은 "이 축이 화면 표면에 있어야 하는가"이고 그것은 A&R 판단이다. 뺀 축도 스냅샷·시리즈에 그대로 남아 언제든 꺼낼 수 있다 |

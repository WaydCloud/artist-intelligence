...
# 🔵 두 갈래 — **러너는 전 레그를 관통했다. 정답지 코퍼스는 수집 중이다** (2026-08-02)

> 📄 이번 세션 이력 = **[`Handoffs/2026-08-01-collector-migration.md`](Handoffs/2026-08-01-collector-migration.md)** · 결정 = **[`docs/DECISIONS.md`](docs/DECISIONS.md) D-059**(이전) · **D-060**(멜론) · 사전 등록 = **[`docs/DRAFT-answer-sheet-corpus.md`](docs/DRAFT-answer-sheet-corpus.md)**.
> 그 앞 = [`Handoffs/2026-08-01-backup-live-and-ip-verdict.md`](Handoffs/2026-08-01-backup-live-and-ip-verdict.md) · D-058.

## 갈래 ① 수집 이전 — ㉱ 병행 운전 중

### 🔑 러너가 08-02 09:17 KST에 **전 레그를 관통했다** (run `30724833099` · **6분**)

| 레그 | 결과 |
|---|---|
| 차트 4레일 | spotify **50/50** · apple **49/49** · youtube **49/49** · shazam **42/42** — **차단 0** |
| sonic | 프리뷰 취득 **2.5분** (콜드 캐시 · 모델 내려받기 포함) |
| yt · 파생 · 리포트 | 전부 성공 · deriveFails **0** · 계약 게이트 **6리포트 CLEAN** |
| 모드 | `MODE = read-only` — 유료 0원 · 1층 쓰기 0 · `main` 푸시 0 |

- ✅ **D-059 ⑩이 닫혔다** — ip-probe가 못 쟀던 `cloud.cp.jku.at`·`essentia.upf.edu` 둘 다 러너에서 열린다.
- ✅ **"아직 안 재진 것"이 전부 재졌다**(수집·파생 레그 · 콜드 캐시 · sonic).

**로컬 vs 러너 (같은 날 08-02)**

| | 로컬 | 러너 |
|---|---|---|
| 차트 | 51시장 · 07-17~08-02 | **동일** |
| social-led | 69 | 68 |
| 두 신호 다 있는 팀 | 144 | 133 |
| 소셜 마지막 날 | 08-01 | **07-31** |

차이는 전부 **유료 레그가 러너에서 꺼져 있어서**다(설계대로). 러너는 07-31까지의 복원된 소셜로 파생했고 로컬은 08-01치를 샀다.

### 🔴 그런데 이 대조는 **우연히** 성립했다

`17 23 * * *`(08:17 KST) 슬롯은 **뜨지 않았다.** 대신 `17 0 * * *`(09:17 KST)이 떴는데, 그때 로컬 09:00 실행이 **아직 안 끝나 있어서**(로컬 완료 09:21) 러너가 복원한 트리에 `run_2026-08-02.json`이 없었다 → 그래서 수집했다.

- 🔺 **예약 워크플로는 실제로 건너뛴다** — D-059 ④가 근거로 든 현상이 우리 레포에서 관측됐다. 설계는 맞았지만 **어정쩡한 분으로도 보장되지 않는다.**
- 🔴 다음 대조가 우연에 기대지 않게 하려면 08:17 슬롯이 실제로 뜨는지부터 봐야 한다. 안 뜨면 대안은 ㉠ 손수 dispatch ㉡ 다른 시각으로 옮김.

### 다음 세션이 할 일 (갈래 ①)

1. **아티팩트로 정확히 대조한다.** 이번에 러너 리포트를 **워크플로 아티팩트**(`runner-reports` · 7일)로 남기게 했다 — 지금까진 실행과 함께 사라져 로그 요약줄("몇 팀")까지만 견줄 수 있었다. `gh run download <id> -n runner-reports` 후 로컬 `report.json`과 나란히.
2. **하루 더 병행**하고 이상 없으면 ㉲ 전환.
3. 🔴 **APIFY(유료 소셜)는 러너에서 한 번도 안 돌았다.** ip-probe에서도 skip이었다 — 전환은 **검증 안 된 레그 하나를 안고** 들어간다. 전환 첫날 로그에서 `social fetched: N tags ok`를 반드시 확인할 것.

**㉲ 전환** — 아래 셋을 **같은 날**. 순서가 어긋나면 하루 $6이 나가거나 하루가 통째로 빈다.

1. 🔴 **`daily-collect.yml`에서 `- cron: "17 23 * * *"` 한 줄을 지운다.** 병행 기간엔 러너가 아무것도 안 써서 08:17에 받은 것이 뒤처져도 버려지지만, **전환 후엔 1층에 쓰므로 어제 보드를 오늘 날짜로 박제한다** — 1층은 재취득 불가다.
2. `RUNNER_MODE=live` · `PUSH_REPORTS=on`
3. `schtasks /Delete /TN "AI-daily-collect" /F`

| 레버 (저장소 변수) | 켜면 | 기본 |
|---|---|---|
| `RUNNER_MODE=live` | 유료 소셜 **+ 1층 쓰기**를 동시에 연다 | 꺼짐 |
| `PUSH_REPORTS=on` | 3층(`report.json`)을 `main`에 되민다 | 꺼짐 |
| `PAUSE=`(아무 값) | 즉시 정지 (`data/live/PAUSE`는 러너에 안 간다) | 꺼짐 |

- 🔴 켜는 값이 `1`이 아닌 이유: **GitHub 표현식의 `==`는 숫자로 캐스팅한다.** `vars.X == '0'`은 **미설정일 때 참**이라 첫 실행에서 가드가 열린 쪽으로 실패했다(D-059 ⑪ · 피해 0).
- 🔴 러너 시계는 `tzutil`로 **KST에 박혀 있다** — `$today`가 스냅샷 파일명·재개 상태·시도 원장·백업 키를 전부 정한다. 크론 표 자체는 여전히 UTC로 읽힌다.

## 갈래 ② 정답지 코퍼스 — 수집 중 (10케이스 중 3종)

> 사전 등록 = [`docs/DRAFT-answer-sheet-corpus.md`](docs/DRAFT-answer-sheet-corpus.md) (**수집보다 먼저 커밋** — 순서가 증거) · 채점기 = [`scripts/canon_ingest.py`](scripts/canon_ingest.py)(오프라인·결정적).

표본 부족은 저지클럽 하나가 아니라 **모듈 전체의 병목**이다(hyperpop 5곡 · ukg 6곡 · moombahton 7곡 · amapiano는 등재 철회의 해제 경로에 "정답지 표본 확대 후 재측"이 명시).
...
`drill` 0은 **설계대로**다 — 전문지 하나(1.0)는 컷오프 2.0에 못 미친다.

### 🔴 실측이 확인 포인트 1을 정면으로 때렸다

`jersey-club` 통과 8 중 **7이 Okayplayer 단독**, `ukg-dnb` 통과 10 중 **9가 DJ EZ 단독**. 두 곳 교차는 케이스당 **1곡**.
**"VIBE 단독"을 고쳤더니 "DJ EZ 단독"이 됐다** — 순환 논증은 막았지만 단일 출처 의존은 남았다. 선택지 셋(㉠ `매우 높음`=1.5 ㉡ 독립 2곳 구조 강제 ㉢ 현행 유지)은 문서 §9에 적었고 **값은 A&R 몫**이다.
- ⚠ 출처 5곳 기준 수치다. **값 조정은 수집 완료 후 한 번에** — 결과를 보고 기준을 고치는 순서가 되면 안 된다.

### 다음 세션이 할 일 (갈래 ②)

- **케이스 7종 수집**: `amapiano` · `citypop` · `easy-listening` · `hyperpop` · `moombahton-tropical` · `njs` · `reggaeton-dembow`. 검색은 이미 돌려 후보 URL이 있다(전문지: Rolling Stone 100 레게톤 · DJ Mag 100 DnB · UKF · NME · Vulture · thefortyfive · Record Collector 100 시티팝 · grayarea 등).
- 기존 3종 출처 보강(특히 `drill`은 둘째 출처가 있어야 통과가 생긴다).
- 🔴 **연도가 대부분 빈다** — §5 연대 편향 표가 아직 작동하지 않는다. 측정 단계에서 iTunes 발매일로 채운다.
- `easy-listening`은 원장상 **내생 케이스**(KR 담론)라 외부 캐논이 없을 수 있다 — 없으면 **없다고 적는다**.
- 수집 방법: 세션이 firecrawl `search` → 적격 판정 → `scrape`(구조화 추출)로 곡 목록을 뽑아 `canon_sources.json`에 적고, 채점은 `python scripts/canon_ingest.py -o ...`(네트워크 0).

## 🔴 열려 있는 결함 하나 — **다크에서 sonic-profile 요약 도형 툴팁이 안 열린다**

세션 끝에 `smoke:tabs`가 잡았다. **결정적으로 재현되고, 원인은 코드가 아니라 데이터다.**

| 데이터 | 결과 |
|---|---|
| 어제(08-01) sonic 리포트 | **PASS 12/12** |
| 오늘(08-02) sonic 리포트 | **FAIL 2건** — `dark/sonic-profile/01 템포·박` · `02 리듬·태그` |

요약 레이더의 축은 6개 그대로 있고 결측도 없다. **값만** 달라졌다:
...
- 🔎 **유력한 선**: 스모크는 `main [data-plot="radar"]`의 `circle[role="button"]` **첫 번째**를 잡는다. 첫 축이 21.7이면 그 마크가 **레이더 중심 가까이** 온다 — 중심부는 다른 축의 마크·기준 링·폴리곤이 몰리는 자리라 히트 타깃이 가려지거나 겹칠 수 있다. 다만 **라이트는 통과**해서 이 설명만으로는 부족하다(같은 기하인데 테마만 다르다).
- ❌ 확인된 것: 내 이번 세션 변경(chart-history 문구·`Tunable` 키)이 원인이 **아니다** — 같은 코드에 어제 데이터를 넣으면 12/12 통과한다.
- 🔴 **그래서 오늘 리포트를 커밋하지 않았다.** 트리의 `modules/*/output`은 어제 상태(=CI 초록)로 되돌려 뒀다. 리포트는 매일 재생성되므로 잃은 것은 없고, 고친 뒤 다시 만들면 된다.
- 재현: `npm run dev -- --port 3100` → `AI_DEV_PORTS=3100 npm run smoke:tabs`. 오늘 데이터는 로컬 daily가 다시 만든다(또는 러너 아티팩트 `runner-reports`).

### ✅ 같이 잡힌 것 하나는 고쳤다 — **표시 이름을 React 키로 쓰고 있었다**

`Tunable.tsx`의 트랙 목록이 `key={m.name}`이었다. 오늘 코호트에 **서로 다른 두 녹음이 같은 표시 이름**을 가진 것이 둘 있었다(`Hearts2Hearts - RUDE!` syncopation 0.6265 vs 0.6492 · `Keyveatz - Key Beats`). 키가 충돌하면 React가 **행을 조용히 빼거나 겹친다** — 콘솔 경고 14건이 그것이었다.
- 🔺 **라벨은 읽으라고 있는 것이지 식별자가 아니다.** 멜론 복귀에서 나온 36·37·38번(문구가 데이터를 앞지른 것)과 같은 자리다 — 이번엔 **표시 문자열이 신원 노릇**을 했다.
- 코호트가 매일 바뀌므로 이건 언제든 다시 온다. 고침은 `${name}#${index}`.

## ✅ 이번 세션에 닫힌 것

1. **수집 이전이 섰다**(D-059) — 결정 넷 · 워크플로 · 시크릿 4 · 러너 전 레그 관통.
2. **멜론 4번째 렌즈 복귀**(D-060) — 100/100 랭크 연속, 07-20 잃어버린 관측도 원본에서 복구. `Melon 1위 = YouTube 1위 = RESCENE LOVE ATTACK`, Spotify는 CORTIS `REDRED` — 1위 레벨 렌즈 발산 재관측.
3. **정답지 코퍼스 사전 등록 + 1차 수집** — 위 갈래 ②.

## 🔴 이번 세션의 결함 여섯 — **전부 "로컬에서는 영원히 초록"**

| # | 무엇 | 왜 로컬에서 안 보이나 |
|---|---|---|
| 33 | 인코딩 짝이 스크립트 중간에서야 걸림 | 콘솔이 cp949라 우연히 됨 |
| 34 | `vars.X == '0'`이 미설정일 때 참 | 로컬에 GitHub 표현식이 없다 |
| 35 | `AI_DRYRUN`이 **복원까지** 마르게 함 | 로컬은 그 변수를 안 쓴다 |
| 36 | 요약 도형이 "네 플랫폼" — 축이 다섯인데 | 렌즈가 넷일 땐 맞는 문장이었다 |
| 37 | "멜론은 아직 연결되지 않아" — 옆 타일이 `Melon 1위`인데 | 데이터와 화면을 대조해야 보인다 |
| 38 | `Spotify에 없는 팀` 힌트가 이름을 손으로 나열 | 값은 이미 멜론을 세고 있었다 |
| (+) | ruff `EXE001` 셔뱅 | **같은 ruff 0.16.0** — 다른 건 파일시스템(Windows는 실행 비트를 안 쓴다) |

> 🔺 앞의 서른두 건은 눈으로 보거나 눌러 보면 나왔다. 이번 것들은 **다른 환경에서 실행하거나 데이터와 화면을 대조해야만** 나온다.
> **이전은 결함을 만드는 일이 아니라 이미 있던 결함을 보이게 하는 일이다.**
...
1. **결정 넷을 받았다**(D-059 ①~④ · 전부 엔지니어 권고안). 파생 3종을 **전부 러너에서** · **이전이 멜론보다 먼저** · **`windows-latest`**(재작성 0) · **`09:17` + 2시간 간격 4회**(정각은 스킵되는 최악 슬롯).
2. **워크플로가 섰다** — `.github/workflows/daily-collect.yml`(15스텝) + `requirements-daily.txt`. 순서는 **복원 → 수집 → 백업**이고, 러너는 매 실행 빈 디스크로 태어나므로 **1층 전 이력을 먼저 되돌린다**(`backup_live.py --restore` · 실측 1,627파일 32초).
   - **게이트를 앞에 세 개 뒀다**: 시크릿 4/4 확인 · 복원된 트리에 **차트 날짜가 2개 이상인지**(하루짜리 시리즈가 리포트로 나가는 것을 막는다) · 되밀기 직전 스키마+`validate_report_data.py`. 🔑 셋 다 "실패가 빨간 X가 아니라 **그럴듯한 산출물**로 나타나는 것"을 막는 자리다.
3. **의존성을 고정했다 — 재현성이 아니라 엔진 지문 때문**(D-059 ⑨). 러너가 다른 librosa로 재면 그날 값은 음악이 아니라 **측정 조건 차이**를 재고 게이트가 그것을 거부한다(D-037·D-038). 올릴 때는 **로컬을 먼저** 올린다.
4. **`ip_probe.py`·`ip-probe.yml` 삭제**(목적 완수 · 판정은 D-058 세션 기록과 §2.2.1에 남는다).

### 🔴 이번엔 실행이 아니라 **환경 가정을 훑어서** 잡혔다 (서른셋째 건)

**모듈 CLI 43줄이 한글을 stdout으로 찍는데, 인코딩 짝이 스크립트 중간(sonic 레그)에서야 걸리고 있었다.** 로컬 콘솔이 cp949라 그 앞의 레그들도 **우연히** 되던 것이다. 러너의 ANSI 코드페이지는 cp949가 아니므로 같은 `print`가 `UnicodeEncodeError`로 레그를 죽인다 — **이전 첫날 `chart-history` 라이브 리포트부터 터졌을 자리다.**
...
### 🔴 그리고 그것을 고치자마자 다음 것이 나왔다 (서른다섯째 건)

**읽기 전용 모드가 제대로 서자 부트스트랩이 아무것도 받지 않았다.** `backup_live.py`가 `dry = args.dry_run or AI_DRYRUN==1`을 **업로드와 복원에 똑같이** 넘기고 있었다 — `--restore`가 나중에 붙으면서(D-058 ⑩) 업로드용 `dry`를 물려받은 것이다. `AI_DRYRUN`이 막으려는 것은 **지출과 바깥으로 나가는 쓰기**이고 **복원은 받는 쪽이라 해당하지 않는다.**
- 🔑 **여기서 트리 게이트가 처음으로 실전에서 값을 했다** — `chart dates=0`으로 잡아 세웠다. 없었으면 "오늘 하루치만 있는" 시리즈가 리포트가 되어 나갔다. **이 워크플로가 막으려고 세운 바로 그 실패다.**
- 회귀 방지: selftest에 디스패치 가로채기 검사 셋(네트워크 0) + **변이 시험**으로 검사가 실제로 문다는 것 확인.
- 🔺 **가드의 이름이 넓다고 적용 범위까지 넓히지 않는다.** 무엇을 막는 가드인지가 범위다. 로컬에서는 영원히 안 보인다 — 로컬은 `AI_DRYRUN`을 설정하지 않고 돈다.
...
### ✅ 멜론 4번째 렌즈가 돌아왔다 (D-060)

수집 플랫폼 **5**. 커버리지 `Spotify 97.4 · Shazam 76.3 · Apple 63.2 · Melon 50.0 · YouTube 28.9`.
🔑 **Melon 1위 = YouTube 1위 = RESCENE `LOVE ATTACK`**, Spotify는 CORTIS `REDRED` — 1위 레벨의 렌즈 발산이 D-017 도입 때와 같은 모양으로 다시 관측됐다.

- **잃어버린 관측 하나를 되찾았다** — 07-20 스토어는 D-018 ⑦의 버그로 지워졌지만 **원본 응답 4장이 살아 있어** 같은 변환으로 복구했다. 재취득 불가한 1층이 되살아났다.
- 🔴 **수집은 세션에서만 된다**(MCP OAuth · 러너가 못 부른다). 다음 수집은 아래 명령 4콜 → `convert-melon`.

```bash
# 멜론 수집 (세션-보조 · D-017) — MCP get_music_chart(DAILY) 를 start 1/31/61/91 로 4콜,
# 응답 원본을 data/live/melon_raw/<날짜>_p{1..4}.txt 로 저장한 뒤:
PYTHONPATH=modules/chart-history/src PYTHONIOENCODING=utf-8 python -m chart_history convert-melon \
  data/live/melon_raw/<날짜>_p1.txt ... p4.txt --store data/live/chart/melon/kr --date <날짜>
#   그다음 analyze · signals · bridge · collect-reports 를 다시 돌린다(daily 와 같은 순서).
```

### 🔴 렌즈를 하나 늘리자 문구 셋이 데이터와 갈라졌다 (서른여섯~서른여덟째 건)

전부 **육안으로만** 잡혔다 — 정적 게이트 5종과 `smoke:tabs` 12건을 **통과한 상태**였다. 문장이 참인지는 데이터를 봐야 알기 때문이다.

| # | 무엇 |
|---|---|
| 36 | 요약 도형이 **"네 플랫폼"**이라고 적고 있었다 — 축이 다섯인 채로(하드코딩 6곳) |
| 37 | "답하지 않는 것"이 **"멜론은 아직 연결되지 않아"** — 바로 옆 타일이 `Melon 1위`를 싣고 있는데도 |
| 38 | `Spotify에 없는 팀` 힌트가 플랫폼 이름을 **손으로 나열**(멜론 누락). 값은 이미 멜론을 세고 있었다 |

- 🔺 **규율**: 문구가 세는 수·이름은 상수로 두지 않는다. 상수로 두면 렌즈가 늘거나 하루 수집이 비는 순간 화면이 **조용히 거짓말을 한다.** D-048의 "원장이 쓰지 말라는 축이 화면에 올라가 있던 것"과 같은 자리 — **데이터와 화면을 나란히 놓고 대조해야만** 보인다.
- 37번은 되돌릴 수 있게 만들었다: 멜론이 **정말로 없는 날**에는 옛 문장이 다시 맞다(세션-보조라 그런 날이 있다).
...
> ⏳ **이전 중이다**(D-059). 러너 워크플로가 섰지만 아직 안 돌았고, **지금 실제로 도는 것은 아래 로컬 Task Scheduler뿐이다.** 둘을 동시에 "가동 중"으로 두는 기간이 병행 운전이며, 그때 러너는 **읽기만 한다**.

- **Task Scheduler `AI-daily-collect`** → `scripts/daily_collect.ps1`. 설정 정본은 [`scripts/register_task.ps1`](scripts/register_task.ps1)(멱등).
- **러너**(예정) → [`.github/workflows/daily-collect.yml`](.github/workflows/daily-collect.yml) · 09:17 KST + 2시간 간격 4회. **같은 `daily_collect.ps1`을 그대로 돌린다**(`shell: powershell` = 5.1).
...
4. ~~**멜론 복구**~~ ✅ **완료**(2026-08-01 · D-060). 4콜 → `convert-melon` → **100/100 랭크 연속**, 07-20 잃어버린 관측도 원본에서 복구. 커버리지·시장 수는 움직였고 **좌측 절단은 안 움직였다**(07-20이 이미 범위 안). 🔴 수집은 세션에서만 된다 — 러너가 못 부른다.

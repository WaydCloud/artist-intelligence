...
7. 저지클럽 **하이햇 롤**과 드릴 후보가 걸려 있던 병목. "정의 먼저 → 수용조건 먼저 → 구현" 순서를 밟았다(RULES §3.1.5.2·§3.1.5.3 → TESTS §7.3 → 구현).
   ...
   - ⚠ **부수 결과 — 유효성 바닥은 격자에 묶여 있다**: HOP을 내리자 같은 곡 `snare_bar_contrast`가 1.347 → **2.003**(Cookiee Kawaii)로 올랐다. **D-037의 1.33은 v4 분포에서 다시 바닥이 아니다** → v4 코호트에서 재도출(RULES §3.8.4.3 형식을 처음부터 다시 밟는다). 값만 옮기면 1.71의 실수를 버전 축에서 반복하는 것.

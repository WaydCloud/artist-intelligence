...
✅ **LABS가 열렸다**(D-052 · `/labs`). 임펄스 원장 10케이스를 화면에 올렸다 — **리포트 계약 밖**이다(`collect-labs.mjs`가 연구 JSON을 빌드 타임에 읽는다). 분리한 이유: 원장은 측정이 아니라 **가설**이고, 증거와 한 표면에 두면 같은 무게로 읽힌다. 경고는 **페이지와 항목 양쪽**에 있고, 확실성 중간 미만 6건은 **빼되 뺐다고 적는다**.
- ⏳ **남은 것 하나**: 원장 문장에 내부 참조가 **4건** 남아 있다(`A2 본편`·`H3`·`H4·H5` 등이 limits에). 화면에서 뜻이 서지 않는다. **원장 내용 수정은 도메인 몫**이라 넘겨 뒀다.
- 🔴 **육안에서 결함 3건을 또 잡았다**(정적 게이트 넷 전부 통과 상태): **확실성을 색으로 그려 낮은 등급이 가장 안 보이던 것**(§7.5 부류) · 링크 글자가 `출처 1`이라 행선지를 안 말하던 것 · 한계 목록이 한 문단으로 뭉치던 것. **스물넷째 건이다.**

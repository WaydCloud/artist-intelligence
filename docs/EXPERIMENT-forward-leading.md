...
### 재시도 감사 (2026-07-30) — 재개 기계장치가 부분 실패에는 안 돌고 있었다

Apple 레그의 스토어프론트 9개 실패를 고친 뒤 **다른 레그도 같은 구조인지** 전수 확인한 결과다. 두 층에 구멍이 있었다.

| 층 | 발견 | 조치 |
|---|---|---|
| **요청** | Kworb 3레그(spotify·youtube·shazam)가 **단발 요청**이었다 — Apple만 고친 것은 운이지 구조가 아니었다 | 재시도를 `chart_history._fetch_bytes` 한 곳으로 모아 Apple·Kworb가 공유(`--attempts` 3 · `--backoff` 2.0). RULES §1 |
| **요청** | `yt_pulse._get`도 단발 요청 | 5xx·네트워크 오류만 재시도, **4xx는 즉시 종료**(403 `quotaExceeded` 재시도는 쿼터를 더 태운다). yt-pulse RULES §1 |
| **오케스트레이션** | 🔴 데일리가 `Save-State $true`를 **무조건** 호출했다. 그래서 `Set-LegResult`가 남긴 `pending` 타깃이 **한 번도 재시도되지 않았다** — 2시간 뒤 시도가 `done=true`를 보고 즉시 no-op으로 나갔다. 재개 기계장치(D-018)는 스크립트가 **중간에 죽은** 경우만 살렸고 **부분 실패**는 못 살렸다 | 무료 수집 레그에 pending이 남으면 하루를 **의도적으로 미완**으로 두어 다음 시도가 실패 타깃만 다시 걷게 한다. **4회 시도(=8시간)** 뒤에는 결손을 확정하고 무엇이 비었는지 로그에 남긴다(무한 재시도는 GAP 경보를 매일 울리게 만든다) |
| **관측** | Kworb 레그가 `2>$null`로 **실패 사유를 삼켰다** — `FAILED (skipped)`만 남아 원인 재현이 따로 필요했다 | Apple 블록과 같이 stderr 마지막 줄을 로그에 남긴다. 판정도 `$?` → `$LASTEXITCODE`(stderr 리다이렉트 시 PS 5.1이 종료코드 0에도 `$?`를 false로 만든다) |

- ~~**유료 소셜 레그는 일부러 제외했다.**~~ ✅ **2026-07-31 승인·편입**(D-054). 태그가 실패하면 하루를 열어 두고 다음 시도가 **실패한 태그만** 다시 산다.
  - 🔴 **켜기 전에 셈법을 고쳤다.** 위 문장이 "일 상한 안"이라고 적어 두었으나 **코드는 그 말을 지키지 못하고 있었다**: 예산을 성공한 fetch만 셌고 실패는 파일을 안 남겨 다음 실행의 지출 복원에서도 빠졌다. 그대로 재시도를 켜면 실패 비용이 상한 밖으로 샌다(최악 $10).
  - **이제 예산은 시도로 센다** — 액터가 돌면 실패해도 돈이 나가므로, 실행 너머로 살아남는 시도 원장(`data/live/social/attempts_<date>.json`)에 **fetch를 부르기 전에** 기록하고 그 총합으로 그날 지출을 복원한다. 성공한 태그는 여전히 다시 사지 않는다(D-018).
  - **하루 총액은 `daily_budget_usd`($3)에서 멈춘다** — 재시도든 첫 시도든 같은 상한에 걸린다. 멈추려면 `data/live/PAUSE`.
- **sonic 프리뷰 레그는 조치 불필요**: 트랙 캐시가 있고 미해석은 `unresolved`로 기록돼 나중에 멱등 복구가 된다(차트와 달리 소급이 가능한 유일한 레그).

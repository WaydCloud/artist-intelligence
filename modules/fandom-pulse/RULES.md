...
| **게시 가속(모멘텀)** | `(최근 절반 게시/일) − (이전 절반)`. 분모는 **창의 달력 날짜**다(0건인 날 포함) — 관측된 날만 세면 "게시/일"이 아니라 "게시/게시가 있던 날"이 된다 | **하중** | `--momentum-min-days` (기본 2) | 케이던스 가속 = 조기 확산 신호 | 단일 fetch는 window 짧음 — 다일 collect가 본선. 창의 **첫날**은 수집 상한에 잘린 경계라 이전 절반이 과소일 수 있음 |
| **Top 공동 해시태그** | 질의 태그 제외 co-occurrence 상위 N | 관습 | `--top-tags` (기본 10) | 공동 태그 = 확산·맥락 도달 | 봇·스팸 태그 혼입 가능 |
| **Top 사운드** | `music` 라벨 상위 N. **UGC 라벨(`… - Original audio`)은 제외**하고 제외 건수를 신뢰도 라인에 적는다 | 관습 | `--top-sounds` (기본 8) | 트렌딩 사운드 = 챌린지 선행(DD). UGC 라벨의 앞부분은 곡이 아니라 **개인 계정명**이라 화면에 실을 수 없다(§5 · AGENTS §5) | music 메타 결측 많음. 제외분만큼 표본이 줄어듦(신뢰도 라인에 표기) |
...
- **일별 라인의 x축은 창의 달력 날짜 전부**다. 관측된 날만 세우면 비어 있는 날이 화면에서 사라지고 선이 그 위를 곧게 지나가 "그동안 계속 그 정도였다"로 읽힌다(2026-07-30 육안 검사: 열흘이 한 칸으로 접혀 있었다). 창 **안쪽**의 빈 날은 결측이 아니라 0건이다 — 표본이 최신순이라 창 안의 게시물은 이미 다 들어와 있다.
...
- **게시물 0**: 크래시 금지. 유효 report + `insights`에 "게시물 없음" 명시. **빈 스냅샷도 시각화 계약을 지킨다**(요약 도형은 빈 막대로 남고, 놓을 차트가 없으므로 구획은 선언하지 않는다).
- `insights`에 **반드시 병기**: (a) 공개 IG 표본 한계(첫 페이지/스크랩 편향), (b) 참여 = 신호일 뿐 **인기·품질 단정 아님**, (c) 단일 fetch면 모멘텀은 짧은 창.
- **시각화 계약 채택**(D-041·D-042·D-043): `summary`(R2) · `questions`(R1) · `charts[].question`(R3) · `inferences`(R4) · `definition`(R5) · `notAnswered`(R7) · `reliability`(R8) · `sections`(구획) 전부 산출. 형식 정본은 [`../../DESIGN.md`](../../DESIGN.md) §7.1, 배치는 [`SPEC.md`](SPEC.md) UX. **문구는 `report.py`의 `_SECTIONS`·`_CHART_META`·`_METRIC_META` 세 표에 모은다** — 계산 코드 사이에 흩으면 카피 검토가 불가능해진다.
- 구획을 선언하려면 차트가 놓인 구획이 **2개 이상**이어야 한다(하나뿐이면 내비게이션이 할 일이 없다). 구획이 사라지면 그 구획의 지표는 첫 구획으로 옮긴다 — 그러지 않으면 화면 어디에도 놓이지 못한 채 조용히 빠진다.
...
- 다루는 것은 **공개 집계 지표**. 팬 개인(닉네임·계정·댓글 원문 등)은 이 모듈 범위 아님 — fetch가 폐기.
  - ⚠ **사운드 라벨로도 새지 않게 한다**: UGC 오디오의 `music`은 `<계정명> - Original audio` 형태라 그대로 차트에 실으면 fetch가 버린 `ownerUsername`이 화면과 커밋된 `report.json`에 다시 나타난다(2026-07-30 육안 검사에서 실제로 막대 4개가 계정명이었다). 집계에는 쓰되 **개별 라벨은 싣지 않는다**.

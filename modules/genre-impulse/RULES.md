...
| jersey-club | 킥 5연타는 rhythm_match 후보이나 하프타임 스네어·하이햇 롤 잠금 | **정답지 확대**(도메인 소유자) 후 재실행. 하프타임 스네어 축(sonic-profile RULES §3.8.2)은 유효성 바닥을 재도출해(1.71 → 1.33, §3.8.4.3) **판정 가능**해졌고, 그 상태에서 사전 등록 기준을 못 넘어 **판별력 없음**으로 판정됐다(정답지 5곡 백분위 88.5·87.4·79.3·66.7·54.0 — 전부 중앙 위지만 상위 20%는 2곡). 기준을 내리지 않는다 — 5곡·컷 하나로는 약한 신호와 없는 신호가 구별되지 않으므로 **정답지를 15~20곡으로 넓혀** 같은 기준으로 다시 잰다. ⚠ **하이햇 롤은 이걸로 안 열린다** — 16칸 격자 해상도 문제라 스템과 무관(별개 변경) |

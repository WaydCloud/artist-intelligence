...
- [ ] **L. (v1.4) 시각화 계약 + 구획**: `scripts/validate_report_data.py`가 **채택 모듈**로 검사(R1~R8) 통과 · 구획 4개 각 차트 ≤3 · 지표 8개 전부 배정 · 요약(`class-mix`)은 `charts`에 중복되지 않음.
- [ ] **M. (v1.4) 부호를 막대에 싣지 않는다**: 선행/지연이 **두 차트**로 갈리고 값이 전부 양수다(음수 막대 0개).
- [ ] **N. (v1.4) 결측을 0으로 그리지 않는다**: 예시 라인의 두 시리즈에서 **관측 없는 날이 `null`**이고, 끊긴 개수가 화면에 표기된다.
- [ ] **O. (v1.4) 상한 표기**: 막대가 30행을 넘으면 신뢰도 라인에 `상위 30팀만 표시`가 붙는다(말없이 자르지 않는다).
- [ ] **P. (v1.5) 두 관문 통과 팀의 이름이 구조로 나간다**(RULES §3.1): `gate-passed` 막대의 팀 수가 지표 `판정 가능 선행`과 같고(30행 상한 아래에서), 교차표 insight의 `양쪽 통과` 목록과 **같은 팀**이다. `--min-posts`를 올리면 셋이 함께 줄고, 통과 0팀이면 차트가 아예 없다(빈 카드 아님).

## 검증 로그 (2026-07-30, v1.4 시각화 계약 + 구획 · D-041·D-042·D-043)

- **L** ✅ `ADOPTED_MODULES`에 `signal-bridge` 추가 → 검사기 CLEAN(전수 6리포트). 구획 4(선행·지연 2차트 / 한 팀 보기 1 / 차트 밖 1 / 기준 1) · 지표 8개 전부 배정 · 질문 4개 앵커 정상.
- **M** ✅ `lead-days` 62팀(양수) · `lag-days` 44팀(절대값). 이전에는 부호 있는 값 하나를 한 막대에 실어 **차트 선행 44팀이 전부 1% 폭**으로 그려지고 있었다(렌더러가 음수를 최소 폭으로 클램프) — 44팀의 차이가 화면에서 사라진 상태였고 정적 게이트 전부를 통과했다.
- **N** ✅ 예시 라인(KiiiKiii): 창 104일 중 소셜 관측 103일 · 차트 관측 14일, 나머지는 `null`(끊긴 지점 91개를 차트가 스스로 표기). 이전에는 차트 강도가 **넉 달간 0으로 그려지다** 수집 개시일에 1로 치솟아 "그때는 차트에 없었다"로 읽혔다(사실은 "아직 수집하지 않았다").
- **O** ✅ `차트 밖에서 도는 팀 264팀 · 상위 30팀만 표시` 등 3개 막대에 상한 표기.
- **육안**(라이트/다크 × 구획 4) ✅ 부제에서 θ 표기 제거(모듈 id는 유지 — 교차 링크가 그것으로 만들어진다) · 요약 도형에서 소셜-온리 264팀을 빼 세 분류의 비교가 눌리지 않게 함.
- **게이트** ✅ ruff · pyright 0 · schema valid · 검사기 CLEAN · 대시보드 typecheck·lint 0 · `smoke:tabs` PASS.

...
4. (bar) act별 대표 평균 일 조회 = **진입 요약** · (bar) act별 조회 합 · (bar) act별 최근 업로드 수 · (line) 다일 축적 시 조회 증분(v2)
...
- **진입부**: 답하는 질문 3개(R1) + 답하지 않는 질문(R7) + **요약 도형 하나**(R2) = `velocity-by-act`(bar, 상위 8팀). 쌓인 조회(누적)와 지금의 속도는 **다른 축**이고, 하중을 받는 쪽은 속도다.
- **구획 2개**(D-043 · 한 구획에 차트 3개까지):

  | 구획 | 질문 | 차트 | 지표 |
  |---|---|---|---|
  | `reach` 쌓인 조회 | 공식 채널에 조회가 얼마나 쌓였나? | `views-by-act` | 추적 팀 · 수집한 영상 · 조회 합 |
  | `fresh` 새 영상 | 지금 새 영상이 도는 팀은 어디인가? | `fresh-by-act` | 가장 빠른 영상의 일 조회 · 새 영상 |

- 차트·지표마다 `question`(R3) · `definition`(R5) · 신뢰도 라인(R8). 추론(R4)은 관측 **아래**에 배지로.

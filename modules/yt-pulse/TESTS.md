...
- [ ] **H. graceful**: 캐시 미해석 act·영상 0 → 크래시 없이 유효 report + 명시. **영상 0에서도 시각화 계약 유지**(요약 빈 막대 · 질문 3개 · 구획 미선언).
- [ ] **L. (v1.1) 시각화 계약 + 구획**: `scripts/validate_report_data.py`가 **채택 모듈**로 검사(R1~R8) 통과 · 구획 2개(쌓인 조회 · 새 영상)에 차트 1개씩 · 지표 5개 전부 배정 · 요약(`velocity-by-act`)은 `charts`에 중복되지 않음 · 새 차트 `fresh-by-act`가 RULES §3 원장에 등재.
- [ ] **M. (v1.1) 추론의 조건**: 업로드 쏠림 문장은 1위가 중앙값의 **2배 이상**일 때만, 속도 쏠림은 **5배 이상**일 때만 나간다(관측이 그렇지 않으면 문장 자체가 없다).

## 검증 로그 (2026-07-30, v1.1 시각화 계약 + 구획 · D-041·D-042·D-043)

- **L** ✅ `ADOPTED_MODULES`에 `yt-pulse` 추가 → 검사기 CLEAN. 라이브 산출(11팀·영상 178개) 기준 구획 2(쌓인 조회 1차트 · 새 영상 1차트) · 지표 5개 전부 배정 · 요약 `velocity-by-act`(상위 8팀)는 `charts`에 없음 · 질문 3개 앵커 정상.
- **M** ✅ 픽스처(9팀·45영상, 업로드 5·5·5…)에서는 **쏠림 문장이 나가지 않는다**(1위 5 = 중앙값 5). 라이브(업로드 23 vs 중앙값 16 = 1.4배)에서도 나가지 않고, 속도 쏠림만 나간다(1위 2,541,463 = 중앙값의 8배). 조건을 넣기 전에는 5·5·5를 놓고 "몰렸다"고 말하고 있었다.
- **H(확장)** ✅ 레코드 0 스냅샷 → 차트 0 · 구획 미선언 · 요약(빈 막대) + 질문 3개 유지 → 검사기 CLEAN.
- **E(재확인)** ✅ 같은 입력 2회 `generatedAt` 제외 동일.
- **육안**(라이트/다크 × 구획 2) ✅ 부제에서 초 단위 타임스탬프 제거 · 라벨에서 `velocity` 걷어냄(`가장 빠른 영상의 일 조회`) · `최근작 조회 합` → `조회 합`.
- **게이트** ✅ ruff · pyright 0 · schema valid · `validate_report_data.py`(+`--selftest`) · `smoke:tabs` PASS.
